...
%% load dams and additional sediment input
    ...
    elseif ~isempty(varargin) && ~(strcmp(varargin{1},'default')) %if data are specified as function input
        ...
        list = {'Wilcock and Crowe','Engelund and Hansen','Yang','Wong and Parker'};
...
%% hydraulic parameters 
    ...
    for i=1:length(NH)
        ...
        if any(n == sources) %check if the node is a source node
            ...
            Qbi_tr(n,n,:) = tr_cap_junction( Fi_r(n,:) , indx_tr_cap , D50(n), Slope(n), Q(n), Wac(n), v(n) , h(n) );
            ...
            if any(any(Qbi_incoming))  % check if there are incoming sediment cascades 
                ...
                [id_sel, ~] = find(QB_tr(:,Network.Upstream_Node{n})>0);  %id of the nodes of the incoming cascades
                ...
                tr_cap = tr_cap_junction( Fi_r(n,:) , indx_tr_cap , D50(n),Slope(n), Q(n), Wac(n),v(n) , h(n) );
                ...
                %choose the transport capacity equation to be used
                Qbi_tr(n,n,:) = tr_limit(n) .* tr_cap_junction( Fi_r(n,:) , indx_tr_cap , D50(n), Slope(n), Q(n), Wac(n), v(n) , h(n) );
