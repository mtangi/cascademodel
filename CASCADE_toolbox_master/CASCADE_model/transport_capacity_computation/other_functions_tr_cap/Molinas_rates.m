function [ Pci ] = Molinas_rates( Fi_r, h, v, Slope, dmi, D50)
...
%%% Fraction of each grain size in the incoming sediment
Pbi = Fi_r; 
               
% Hydraulic parameters in each flow percentile for the current reach
Dn = (1+(GSD_std(Fi_r,dmi)-1)^1.5)*D50; %scaling size of bed material
...
Zeta = 2.8*FR^(-1.2).*  GSD_std(Fi_r,dmi)^(-3); 
...
frac1 = (Pbi.*( (dmi./Dn).^alpha + Zeta*(dmi./Dn).^beta )); % Nominator in EQ 23, Molinas and Wu (2000) 
Pci = frac1./(sum(frac1));
